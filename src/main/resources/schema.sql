...
DROP TABLE IF EXISTS `over_time_request`;
...
CREATE TABLE IF NOT EXISTS job_position (
    job_position_id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, -- Numeric ID for job position
...
-- INSERT INTO job_position (position_name, position_description, min_salary, max_salary) VALUES
-- ('Entry Level', 'Interns, Trainees, Clerks, and Administrative Assistants', 10000, 20000),
-- ('Professional', 'Specialists, Analysts, Engineers, and Senior Staff', 10000, 20000),
-- ('Supervisor', 'Supervisors and Team Leads', 10000, 20000),
-- ('Senior Manager', 'Vice Presidents (VPs), Directors, and Senior Leaders', 10000, 20000),
-- ('Executive', 'CEO (Chief Executive Officer), CFO (Chief Financial Officer), and other C-suite executives', 10000, 20000);
...
CREATE TABLE IF NOT EXISTS over_time_request (
    request_id INT UNSIGNED PRIMARY KEY,
    target_date DATE NOT NULL,
...
CREATE TABLE IF NOT EXISTS position_bonus (
    ...
    PRIMARY KEY (job_position_id, bonus_id)
);
...
CREATE TABLE IF NOT EXISTS position_deduction (
    ...
    PRIMARY KEY (job_position_id, deduction_id)
);
